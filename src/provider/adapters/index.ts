...
export async function sendRequest(
  ...
  log.debug('request_adapter', { model: inner.model, adapter: adapter.name, stream: inner.stream });
  
  const url = adapter.buildUrl(apiBase, inner.model, inner.stream);
  const headers = adapter.buildHeaders(apiKey, proxySecret);
  const body = adapter.buildBody(inner, apiKey);
